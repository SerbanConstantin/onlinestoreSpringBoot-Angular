...
public class OrderController {
    ...
    public ResponseEntity<OrderMain> cancel(@PathVariable("id") Long orderId, org.springframework.security.core.Authentication authentication) {
    ...
    public ResponseEntity<OrderMain> finish(@PathVariable("id") Long orderId, org.springframework.security.core.Authentication authentication) {
        if (authentication.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_CUSTOMER"))) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
